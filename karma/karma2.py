...
def check_user(user):
    check = user.submissions.new()
    try:
        if check:
            return True
    except Exception:
        print("There is no user with the name" + user + ". Please try again.")
        return False
...
def print_comparison(sub_list):
    if sub_list[0]["karma"] > sub_list[1]["karma"]:
        str_user_one = ("/" + sub_list[0]["user"] + "/'s post titled '" + sub_list[0]["Title"] + "' got them " +
                        sub_list[0]["karma"] + ".\n")
        str_user_two = ("Whereas, /" + sub_list[1]["user"] + "/'s post titled '" + sub_list[1]["Title"] +
                        "' got them " + sub_list[1]["karma"] + ".\n")
        karma_dif = int(sub_list[0]["karma"]) - int(sub_list[1]["karma"])
        str_dif = ("The difference in karma between /" + sub_list[0]["user"] + "/'s post and /" +
                   sub_list[1]["user"] + "/'s post was" + str(karma_dif))
        print(str_user_one + str_user_two + str_dif)
    elif sub_list[1]["karma"] > sub_list[0]["karma"]:
        str_user_one = ("/" + sub_list[1]["user"] + "/'s post titled '" + sub_list[1]["Title"] + "' got them " +
                        sub_list[1]["karma"] + ".\n")
        str_user_two = ("Whereas, /" + sub_list[0]["user"] + "/'s post titled '" + sub_list[0]["Title"] +
                        "' got them " + sub_list[0]["karma"] + ".\n")
        karma_dif = int(sub_list[1]["karma"]) - int(sub_list[0]["karma"])
        str_dif = ("The difference in karma between /" + sub_list[1]["user"] + "/'s post and /" +
                   sub_list[0]["user"] + "/'s post was" + str(karma_dif))
        print(str_user_one + str_user_two + str_dif)
    elif sub_list[1]["karma"] == sub_list[0]["karma"]:
        str_user_one = ("/" + sub_list[0]["user"] + "/'s post titled '" + sub_list[0]["Title"] + "' got them " +
                        sub_list[0]["karma"] + ".\n")
...
def again():
    while True:
        answer = input("Would you like to compare more redditors? Y/N\n")
        if answer.lower() == "y":
            return
        elif answer.lower() == "n":
            exit(0)
